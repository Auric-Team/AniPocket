            ...
            <div className="flex overflow-x-auto gap-4 pb-6 pt-2 px-4 md:px-2 scrollbar-hide snap-x">
                {history.map((item) => (
                    <div key={`cw-${item.id}`} className="relative flex-shrink-0 w-[240px] md:w-[280px] snap-start group animate-in fade-in transition-all">
                        ...
                        <div className="relative bg-[#0a0a0f] border border-white/5 rounded-xl overflow-hidden transition-all duration-500 group-hover:border-[#f43f5e]/50 shadow-[0_4px_20px_rgba(0,0,0,0.5)] group-hover:shadow-[0_0_30px_rgba(255,176,0,0.15)] transform group-hover:scale-[1.02]">

                            <Link href={`/watch/${item.id}?ep=${item.episodeId}&num=${item.episodeNumber}`} className="block relative aspect-video bg-[#13131c]">
                                ...
                                {/* Overlay Gradient */}
                                <div className="absolute inset-0 bg-gradient-to-t from-[#0a0a0f] via-[#0a0a0f]/40 to-transparent opacity-90 z-10" />
                                ...
                                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20 bg-black/40 backdrop-blur-[2px]">
                                    <div className="w-12 h-12 bg-[#f4f4f5] rounded-full flex items-center justify-center text-[#0a0a0f] shadow-[0_4px_20px_rgba(255,255,255,0.2)] transform scale-90 group-hover:scale-100 transition-transform duration-500">
                                ...
                                <div className="absolute bottom-3 left-3 right-3 z-30 flex items-center justify-between">
                                    <span className="bg-[#f43f5e] text-[#0a0a0f] text-[10px] font-bold px-2 py-0.5 rounded-full shadow-[0_2px_10px_rgba(255,176,0,0.3)]">
                                        EP {item.episodeNumber}
                                    </span>
                                    {/* Simulated subtle progress bar */}
                                    <div className="w-1/2 h-1 bg-white/20 rounded-full overflow-hidden">
                                        <div className="h-full bg-[#f43f5e] rounded-full w-[45%]" />
                                    </div>
                                </div>
                            </Link>

                            {/* Meta Data Block */}
                            <div className="p-3.5 relative z-20 bg-[#13131c] border-t border-white/5 flex gap-2 justify-between items-start">
                                <Link href={`/watch/${item.id}?ep=${item.episodeId}&num=${item.episodeNumber}`} className="flex-1 block">
                                    <h3 className="font-semibold text-[#f4f4f5] text-[14px] line-clamp-1 group-hover:text-[#f43f5e] transition-colors pr-2">
                                        {item.title}
                                    </h3>
